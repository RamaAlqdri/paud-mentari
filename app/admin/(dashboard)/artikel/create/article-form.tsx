...
export default function ArticleForm() {
  ...
  const onSubmit = async (data: ArticleFormValues) => {
    ...
    try {
      const result = await createArtikel(formData);

      if (result.success) {
        setResultMessage({ type: "success", text: "Artikel berhasil diterbitkan." });
        reset();
      } else {
        setResultMessage({ type: "error", text: result.message || "Gagal menerbitkan artikel." });
      }
    } catch (err) {
